...
const duplicatedImages = [...DEMO_IMAGES, ...DEMO_IMAGES, ...DEMO_IMAGES];

export function Hero() {
  return (
    <section id="hero" className="relative h-screen overflow-hidden">
      {/* Container Principal */}
      <div className="relative mx-auto max-w-5xl px-6 flex items-center h-full">

        {/* Conteúdo de Texto e Logo */}
        <div className="grid lg:grid-cols-2 gap-10 items-center w-full">
            ...
            <div className="flex flex-col items-center justify-center gap-4 lg:flex-row lg:justify-start">
             ...
             <div className='flex items-center justify-center gap-4 lg:justify-start pt-4'>
                <div className='flex -space-x-2'>
                    <Image src={"https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=900&auto=format&fit=crop&q=60"} alt="User 1" width={40} height={40} className="rounded-full border-2 border-background" data-ai-hint="man portrait"/>
                    <Image src={"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=900&auto=format&fit=crop&q=60"} alt="User 2" width={40} height={40} className="rounded-full border-2 border-background" data-ai-hint="woman portrait" />
                    <Image src={"https://images.unsplash.com/photo-1449824913935-59a10b8d2000?w=900&auto=format&fit=crop&q=60"} alt="User 3" width={40} height={40} className="rounded-full border-2 border-background" data-ai-hint="man face" />
                    <Image src={"https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=900&auto=format&fit=crop&q=60"} alt="User 4" width={40} height={40} className="rounded-full border-2 border-background" data-ai-hint="woman face" />
                </div>
                <div>
          ...
          {/* Lado Direito: Logo Grande (agora controlada pelo grid) */}
          <div className="relative h-full w-full min-h-[600px] flex-1 hidden lg:flex items-center justify-center">
            <Logo className="w-full h-full drop-shadow-2xl" />
          </div>
        </div>

        {/* Camada de Fundo: Carrossel e Gradiente */}
        <div className="absolute inset-0 -mx-4">
          {/* Camada de Gradiente por cima do carrossel */}
          <div aria-hidden className="absolute z-[1] inset-0 bg-gradient-to-r from-background from-40%" />

          {/* Camada do Carrossel de Imagens */}
          <div className="relative h-full w-full">
            ...
            <motion.div
              ...
              animate={{ x: ["-100%", "0%"] }}
              transition={{ ease: "linear", duration: 90, repeat: Infinity, delay: 2 }}
            >
              {duplicatedImages.map((src, index) => (
                <div
                  key={`marquee-2-${index}`}
